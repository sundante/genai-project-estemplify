...
export function AssumptionsTab() {
  const { resetAssumptions } = useWorkspaceStore()
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-lg font-bold text-slate-800 dark:text-white">Assumptions</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Assumptions, dependencies, and FinOps notes.</p>
        </div>
        <SectionActions section="assumptions" onReset={resetAssumptions} />
      </div>
