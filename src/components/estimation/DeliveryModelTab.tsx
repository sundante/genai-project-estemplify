...
export function DeliveryModelTab() {
  const { resetDeliveryModel } = useWorkspaceStore()
  return (
    <div className="space-y-8">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-lg font-bold text-slate-800 dark:text-white">Delivery</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">Delivery model, governance, support, and rollout planning.</p>
        </div>
        <SectionActions section="delivery" onReset={resetDeliveryModel} />
      </div>
